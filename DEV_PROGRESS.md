...
12. **Next architecture task:** address the action-bar bag-instance gap in ClassicAPI itself (expose exact itemID and preferably GUID/location for that action descriptor), then integrate that route in Cooline and runtime-prove it before removing the broad native item-discovery fallback.
...
## Exact Next Step
Begin the narrow ClassicAPI action-descriptor identity addition required by the proven Cooline 3.0 design: for action-bar item entries that currently surface as `"item", nil`, expose exact itemID and preferably stable GUID/location when the action descriptor represents a bag instance. Keep the change scoped to the smallest ClassicAPI API surface needed by Cooline; do not redesign unrelated action APIs. Then update Cooline to consume the new exact identity, retain the native fallback until this new route passes static/compiler checks and user runtime validation, and only remove the broad native item-discovery fallback after that proof. Current remaining validation debt (not blockers for starting this API work): direct ITEM binding / `C_Item.UseItemByName` and the SCRM/pfUI conditional `/use` route were not exercised in this session.
