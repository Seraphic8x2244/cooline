...
- Current dev branch head before this status commit: `630705170670209c4476ecd68296a4e6e9a57241` (`Remove temporary Lua 5.0.2 check`). Runtime implementation commit: `b328f7b45261767fb913441e156663beed2fcfe7` (`Retry exact spell cooldown observation`). The current handoff is the commit containing this file on `dev`; verify the actual remote `dev` head before new work.
...
- Still to validate before changing the item architecture: an item placed directly on an action-bar item action (distinct from a macro action); direct ITEM binding / `C_Item.UseItemByName`; supported SCRM/pfUI conditional `/use` where available; and, if convenient, one runtime confirmation that a shared cooldown displays only the used representative rather than duplicate item icons.
...
11. **Targeted regression/recovery gate passed in the current requested test session:** spell cooldowns are appearing again; item/trinket plus potion cooldowns work; failed-cast pulse works; `/reload` and instance zoning reconstruction work; and an action-bar `/use 13` macro works. Complete the remaining direct item-action / binding / supported conditional-use matrix before changing the item architecture. Earlier branch-unverified 3.0 tests do not count.
12. **After the remaining exact-route matrix passes:** address the action-bar bag-instance gap in ClassicAPI itself (expose exact itemID and preferably GUID/location for that action descriptor), then only remove the broad native item-discovery fallback after the new route is code- and runtime-proven.
13. Keep `main` on stable `2.1.2` and `native-2.1` unchanged until the 3.0 line is explicitly accepted for promotion.
...
## Exact Next Step
Keep the same requested `3.0.4-dev` build and finish only the remaining item-entry-route coverage before new product changes. Test an **item placed directly on an action-bar slot** (not a macro), then a direct ClassicAPI ITEM binding / `C_Item.UseItemByName` route, and the supported SCRM/pfUI conditional `/use` route where available. Shared item cooldowns are intentionally represented by exactly one locked item/icon per cooldown signature; if convenient, confirm at runtime that using one of two shared-cooldown items produces only that one representative icon. Once these implemented routes pass, the next code task is the narrow ClassicAPI action-descriptor identity addition for bag-instance item actions, followed by Cooline integration and runtime proof before deleting the broad native item-discovery fallback.
