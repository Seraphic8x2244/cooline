...
## Current
- Branch: `dev`.
- Version: `3.0.4-dev` in `Cooline.toc`.
- Current dev branch head before this status commit: `d62c13b70bcff154a3fb61e3a2f71bb1047cc785` (`Fix and clarify addon build versioning rule`). Runtime implementation remains `b328f7b45261767fb913441e156663beed2fcfe7` (`Retry exact spell cooldown observation`); subsequent Cooline commits are documentation/rulebook-only, so the addon product remains the same `3.0.4-dev` runtime tree. The current handoff is the commit containing this file on `dev`; verify the actual remote `dev` head before new work.
...
- ClassicAPI audit baseline: `Seraphic8x2244/ClassicAPI` `master` at `7ab32df2aadc2171100aac859154085fcaed56b2`.
- ClassicAPI bag-action identity candidate: branch `cooline-action-item-identity` at `7e4c7d4b35a30a6fb83186ed7d062ea4cbb3a1e0`. GitHub Actions run `36073680632` completed successfully and uploaded dev artifact `ClassicAPI-7e4c7d4b35a30a6fb83186ed7d062ea4cbb3a1e0` (SHA-256 `a290c0ba7d41627f2c1eef3a4250e044ac4ad854294760e508b61f128fbf8ee8`). ClassicAPI `master` remains unchanged.
...
- Current scope boundary: 2.1 is finished/released/preserved. 3.0 Stage 3 is at `3.0.4-dev`. Exact item-use observation remains implemented for the proven routes. The first spell runtime attempt exposed a timing defect: `UNIT_SPELLCAST_SUCCEEDED` can arrive before the direct ClassicAPI cooldown query reflects the newly started cooldown. `3.0.4-dev` now retains the exact successful spellID through a bounded retry and one recovery reconciliation instead of dropping it. The targeted 3.0.4 runtime check now reports spells appearing on the bar again, with item/trinket and potion cooldowns also working. The user's reply did not independently restate the load-message/version check, so record these results as the current 3.0.4 test-session pass while retaining the rule that **all pre-3.0.4 3.0 runtime observations are branch-unverified and do not count as validation**. Native item discovery remains deliberately retained only as transitional recovery / ambiguity coverage, especially for action-bar bag-instance entries whose itemID ClassicAPI still does not expose.
...
- `630705170670209c4476ecd68296a4e6e9a57241` — Remove the temporary Lua 5.0.2 workflow after the `3.0.4-dev` compiler pass; product files are unchanged from the checked runtime implementation.
- `b0083d74b6ff0e9762c2ae80dbd9e3acd5334cd7` — Add the temporary Lua 5.0.2 compiler workflow used to check the `3.0.4-dev` product tree.
- `b328f7b45261767fb913441e156663beed2fcfe7` — Fix exact spell cooldown observation timing: retain successful spellID/name through a bounded direct-query retry and one recovery reconciliation; bump to `3.0.4-dev`.
...
- The earlier `3.0.3-dev` handoff lacked a real compiler pass because the checker could not be retrieved into the direct execution environment. For `3.0.4-dev`, the repo's previously proven temporary GitHub Actions workflow was restored instead.
- GitHub Actions run `36045308278` successfully downloaded the official Lua 5.0.2 source archive, verified SHA-256 `a6c85d85f912e1c321723084389d63dee7660b81b8292452b190ea7190dd73bc`, built Lua 5.0.2, and ran `luac -p` successfully against `Cooline.lua` and all locale Lua files for the exact `3.0.4-dev` product tree.
- The temporary compiler workflow was removed immediately after the pass. Cleanup commit `630705170670209c4476ecd68296a4e6e9a57241` changes only the workflow file, so the product files remain identical to checked runtime commit `b328f7b45261767fb913441e156663beed2fcfe7`.
...
## Current Issues
- The earlier missing-spell regression is no longer reproducing in the current 3.0.4 test session: the user reports spells appearing on the bar again, and item/trinket plus potion cooldowns also working. Treat all pre-`3.0.4-dev` 3.0 gameplay observations as branch-unverified and invalid for validation.
- Root cause identified in the `3.0.3-dev` spell path: Cooline queried `C_Spell.GetSpellCooldown(spellID)` immediately inside `UNIT_SPELLCAST_SUCCEEDED`. ClassicAPI fires that event directly from its spell-success/SPELL_GO path, which can precede the client cooldown state becoming visible. If the immediate query returned no active cooldown, Cooline discarded the successful spellID; later `SPELL_UPDATE_COOLDOWN` refreshed only already-active spell entries, so the cooldown could be lost completely.
- `3.0.4-dev` fixes that timing hole by retaining the exact successful spellID/name for up to one second, retrying the direct query on the runtime driver and `SPELL_UPDATE_COOLDOWN`, and using one existing spellbook reconciliation only as a timeout recovery backstop. This fix is compiler-checked and the current test session reports spell cooldowns working again; exact build provenance remains tied to the mandatory `3.0.4-dev` load-message check from the test instructions.
- ClassicAPI `master` still returns `"item", nil` for a `UseAction` bag-instance action. The compiler-checked candidate branch `cooline-action-item-identity` now asks vanilla's own `GetActionText` handler for the bag-action name and returns an itemID only when all matching carried instances agree on one ID; same-name/different-ID ambiguity remains `nil` rather than guessed. This candidate does not expose GUID/location and still performs a bounded carried-item scan in C++, so it is a conservative bridge rather than a proven direct decode of the opaque action descriptor. Cooline intentionally retains its native item-discovery fallback until runtime proof and a follow-up decision on whether this bridge is sufficient.
- The `3.0.4-dev` Cooline product tree has a real Lua 5.0.2 compiler pass from GitHub Actions run `36045308278`; no Cooline runtime Lua has changed since that checked tree.
...
- Requested version/runtime: **`3.0.4-dev`**, runtime implementation `b328f7b45261767fb913441e156663beed2fcfe7`, product-identical checked cleanup head before the status commit `630705170670209c4476ecd68296a4e6e9a57241`.
- Targeted regression result: user reports spell cooldowns are appearing on the bar again; item/trinket cooldowns work; potion/consumable cooldown works.
- This validates the behavior that had regressed in the first 3.0 spell attempt and reconfirms the basic item path in the current requested test session.
- Provenance note: the test instructions required the client load message `Cooline 3.0.4-dev loaded.` before interpreting results, but the user's result message did not independently restate that line. Do not attach these results to any earlier 3.0 build.
- Additional current-session passes: failed-cast pulse; `/reload` reconstruction; instance zoning/loading-screen reconstruction; and an action-bar macro containing `/use 13`, which validates the ordinary macro path through equipped-slot use. Shared-cooldown behavior is intentionally one representative icon per cooldown signature, preferring the exact captured item identity when available.
- Additional current-session passes: an item placed directly on an action-bar item action works; shared-cooldown behavior was runtime-confirmed to show the item actually used as the single representative icon.
- The user does not currently have a conditional `/use` macro available to exercise the SCRM/pfUI conditional path. Treat that route as untested coverage, not a failure. Direct ITEM binding / `C_Item.UseItemByName` remains untested unless independently exercised later.
- The action-bar bag-instance route remains the active architecture gate on ClassicAPI `master`. Candidate ClassicAPI branch `cooline-action-item-identity` is now compiler/build-checked but not runtime-tested. Existing Cooline `3.0.4-dev` already consumes a non-nil `GetActionInfo` itemID, so the candidate can be runtime-proven without a new Cooline build; the fallback remains active for `nil`/unresolved cases.
...
8. **Implemented in Cooline:** item-by-ID action entries use exact identity. On ClassicAPI `master`, bag-instance action entries still return `"item", nil`; for those, and for one exact-route timeout recovery pass, the legacy native item-discovery scan remains intentionally available. **ClassicAPI candidate implemented/checked:** `cooline-action-item-identity` at `7e4c7d4b35a30a6fb83186ed7d062ea4cbb3a1e0` supplies an itemID for unambiguous bag-action name matches and passed Win32 build run `36073680632`; runtime proof is pending.
...
10. **Spell timing correction implemented/checked:** `3.0.4-dev` retains the exact successful spellID through a bounded retry so a too-early success event cannot permanently lose the cooldown. GitHub Actions run `36045308278` passed the real Lua 5.0.2 compiler check.
...
12. **ClassicAPI candidate implemented/checked:** the narrow `cooline-action-item-identity` branch resolves bag-action names through vanilla `GetActionText` and exposes an itemID only when carried matches are unambiguous. The final branch head `7e4c7d4b35a30a6fb83186ed7d062ea4cbb3a1e0` passed GitHub Actions Win32 build run `36073680632`. This is not yet user runtime-tested and is not a direct GUID/location decode.
13. **Current gate:** runtime-prove the candidate API with existing Cooline `3.0.4-dev`; Cooline already takes the exact-item branch when `GetActionInfo` returns a numeric itemID. Do not remove any fallback yet. After the proof, decide whether the conservative C++ bridge is sufficient or whether direct opaque-descriptor/GUID reversing is still required before fallback removal.
14. Keep `main` on stable `2.1.2` and `native-2.1` unchanged until the 3.0 line is explicitly accepted for promotion.
...
## Exact Next Step
Runtime-test ClassicAPI branch `cooline-action-item-identity` at exact head `7e4c7d4b35a30a6fb83186ed7d062ea4cbb3a1e0` using build artifact `ClassicAPI-7e4c7d4b35a30a6fb83186ed7d062ea4cbb3a1e0` from successful workflow run `36073680632`, while keeping the already-tested Cooline `3.0.4-dev` addon unchanged. Put a bag item directly on action-bar slot 1 and run `/script local t,id=GetActionInfo(1); DEFAULT_CHAT_FRAME:AddMessage(tostring(t).." "..tostring(id))`; the target result is `item <numeric itemID>`, not `item nil`. Then use that action and confirm Cooline still shows the correct item cooldown/identity. This proves the new ClassicAPI read path separately from Cooline's retained fallback. Do not remove the fallback or bump Cooline yet. If the API returns nil or wrong identity, investigate the ClassicAPI candidate. If it returns the correct numeric ID and Cooline behaves correctly, record the runtime pass and decide whether this conservative C++ bridge is sufficient for Stage 3 or whether direct descriptor/GUID reversing is still required before fallback removal. Remaining non-blocking validation debt: direct ITEM binding / `C_Item.UseItemByName` and the SCRM/pfUI conditional `/use` route were not exercised in this session.
