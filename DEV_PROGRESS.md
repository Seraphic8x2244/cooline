...
- Still to validate before changing the item architecture: spell failure pulse; `/reload` / zoning reconstruction of an active spell cooldown; action-bar item use; direct ITEM binding / `C_Item.UseItemByName`; ordinary macro `/use`; supported SCRM/pfUI conditional `/use` where available; and representative shared-cooldown behavior.
...
11. **Targeted regression gate passed in the current requested test session:** spell cooldowns are appearing again, and item/trinket plus potion cooldowns work. Complete the remaining recovery/failure and macro/binding/action/shared-cooldown matrix before changing the item architecture. Earlier branch-unverified 3.0 tests do not count.
...
## Exact Next Step
Keep the same requested `3.0.4-dev` test build and finish the remaining Stage 3 runtime matrix before making new product changes. Verify spell failure-pulse behavior and `/reload` / zoning reconstruction of an active spell cooldown. Then exercise item activation through an action-bar button, a direct ITEM binding / named-use route, an ordinary `/use` macro, and the supported SCRM/pfUI conditional `/use` route where available, plus one representative shared-cooldown case. The current action-bar bag-instance path may still use the deliberate native fallback because ClassicAPI can return `"item", nil`; do not remove that fallback yet. If these implemented routes pass, the next code task is the narrow ClassicAPI action-descriptor identity addition for bag-instance item actions, followed by Cooline integration and runtime proof before deleting the broad native item-discovery fallback.
