...
- `C_Item.UseItemByName` is runtime-covered: both `/use [Happy Fun Rock]` from a Ctrl-clicked bag-item link and manually typed `/use happy fun rock` worked and Cooline tracked the bag item. The equipped plain-name command `/use vanquished tentacle of c'thun` also resolved the equipped item correctly; the item was already on cooldown, so the client returned the expected cooldown error, confirming the equipped-use route was reached. The earlier failure with the Ctrl-clicked equipped hyperlink form is therefore limited to SCRM/link-string resolution, not Cooline or the equipped-use route.
