...
## Current Issues
- Closure review found no concrete remaining Cooline runtime or architecture blocker for 3.0 promotion. The only release-facing issue identified is documentation: `README.md` is still written for the native/best-effort model and does not state that 3.0 requires ClassicAPI or describe the narrower remaining bag-instance fallback accurately.
...
- `3.0.5-dev` passed the real Lua 5.0.2 compiler check in GitHub Actions run `36075809563`; `Cooline.lua` and all locale Lua files compiled successfully. The temporary workflow was removed immediately afterward.
...
- `C_Item.UseItemByName` is now runtime-covered: typing `/use [Happy Fun Rock]` in chat using a Ctrl-clicked inventory item link worked and Cooline tracked it. The equipped Ctrl-clicked `/use [Vanquished Tentacle of C'Thun]` command did not work, but source review shows SCRM's equipped-item fallback compares a plain item name while a Ctrl-click supplies the full item-link string; if its Nampower fast lookup does not resolve that link, it falls through to bag-only `C_Item.UseItemByName`. Because `/use 13` already passes through `UseInventoryItem`, treat this as an SCRM equipped-link resolution issue, not a Cooline failure.
...
15. **Closure review complete:** no concrete Cooline runtime or architecture blocker remains. `C_Item.UseItemByName` is now runtime-covered by the successful inventory-linked `/use [Happy Fun Rock]` test. The failed equipped linked `/use [Vanquished Tentacle of C'Thun]` attempt is attributable to SCRM equipped-link resolution, while Cooline's equipped `UseInventoryItem` route is already user-proven by `/use 13`.
