...
- Current dev branch head before this correction status commit: `03f5a2f9d456cb269175d555d90bca024a64707b`; that commit changed only `DEV_PROGRESS.md` and incorrectly pointed work into ClassicAPI. Runtime implementation remains `b328f7b45261767fb913441e156663beed2fcfe7` (`Retry exact spell cooldown observation`). The current handoff is the commit containing this file on `dev`; verify the actual remote `dev` head before new work.
...
4. **Do not remove native item discovery for `UseAction` bag-instance entries while `GetActionInfo` returns nil itemID.** With the current public ClassicAPI surface this is an external limitation, not a task to modify ClassicAPI from this project. Retain the narrow fallback in Cooline.
...
- The action-bar bag-instance route remains a known ClassicAPI identity gap: `GetActionInfo(slot)` can return `"item", nil`. Current Cooline intentionally falls back to native discovery for that route until ClassicAPI exposes exact identity.
...
12. **Accepted external limitation:** the action-bar bag-instance identity gap belongs to the current ClassicAPI public surface and is not a Cooline repository task. Keep the narrow native item-discovery fallback for that route and the one documented timeout-recovery pass. Do not edit ClassicAPI from this project.
13. **Current Cooline runtime gate:** the exercised `3.0.4-dev` routes now pass, including direct action-bar item use and shared-cooldown identity selecting the item actually used. The unavailable conditional `/use` route and direct ITEM binding remain untested coverage, not known failures.
...
## Exact Next Step
Stay entirely within `Seraphic8x2244/cooline`. Treat the current ClassicAPI bag-instance `GetActionInfo(slot) -> "item", nil` behavior as an external limitation and retain Cooline's narrow native item-discovery fallback for that action route plus the documented one-pass timeout recovery. Do not edit ClassicAPI. The current `3.0.4-dev` runtime gate has passed all routes the user could exercise in this session; direct ITEM binding / `C_Item.UseItemByName` and the SCRM/pfUI conditional `/use` route remain untested coverage only. Before the next addon runtime change, identify the next Cooline-local Stage 3 cleanup/architecture delta and bump the TOC version for that new build.
